...
class Sync_SugarCRM {
  ...
  function push_user($user_info, $sugarcrm_module) {
    try { 
	  ...
	  add_user_meta( $user_info->ID, 'Sugar_ID',$response->id);
	  add_user_meta( $user_info->ID, 'Sugar_Class',$sugarcrm_module);
  ...
  function editCRM_user($user_info,$sugarcrm_module,$crmid=null) {
	$email = $user_info->data->user_email;
	
    try { 
	
      // Get count of users from SugarCRM with same username
      //$response = $this->plugin->client->__soapCall('get_entries_count',array('session'=>$this->plugin->session_id,'module_name'=>$sugarcrm_module,'query'=>$table, 'deleted'=>0));
    $response = $this->plugin->client->__soapCall('get_entry',array(
		'session'=>$this->plugin->session_id,
		'module_name'=>$sugarcrm_module,
		'id'=>$crmid,
		'select_fields' => array('id','first_name','last_name','contact_id',
	...
	$lconverted = $response->entry_list['0']->name_value_list['3']->value;

	$lcontactid = $response->entry_list['0']->name_value_list['4']->value;
	  ...
	  if (count($response->entry_list) > 0) {
		
		// Edit user to SugarCRM
		$user_arr  = $this->get_soap_name_value_list_edit($user_info, $sugarcrm_module,$crmid);
		...
		$Sugar_Class = get_user_meta($user_info->ID, 'Sugar_Class');
		$Sugar_Class = $Sugar_Class['0'];
		
		// if lead has been converted into contact
		if($lconverted && $Sugar_Class=='Leads'){
			// updating contact as well
			$user_arr['0'] =array('name' => 'id',         'value' => $lcontactid );
			
			$response = $this->plugin->client->__soapCall('set_entry',array('session'=>$this->plugin->session_id,'module_name'=>'Contacts','name_value_list'=>$user_arr));
			
		}
		
		$this->messages[] = 'SugarCRM User updated: '.sizeof($response->result_count);
      }
  ...
  /*
  * Edit user data setting
  *
  */
  
  function get_soap_name_value_list_edit($user_info, $sugarcrm_module,$id) {
